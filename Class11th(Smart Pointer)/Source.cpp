...
#pragma region 스마트 포인터
	...
	// 포인터를 시뮬레이트하는 동시에 자동 메모리 관리 또는 경계 확인과 같은
	// 추가 기능을 제공하는 추상 데이터 유형입니다. 

#pragma region Unique 포인터

	// 특정한 객체를 하나의 스마트 포인터만 가리킬 수 있도록 하는 포인터입니다. 
...
#pragma region 순환 참조 
	...
	// 참조 개수가 0으로 떨어지지 않아 포인터 객체가 소멸되지 않는 상황입니다. 
...
#pragma region  Weak 포인터
	// 자원 객체를 소유하지 않기 때문에 std::shared_ptr로 관리되는 자원 객체를 가리켜서 참조 개수에 영향을
	// 미치지 않게 하는 포인터입니다. 
